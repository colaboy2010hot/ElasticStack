...
cluster.name | 클러스터 이름     | 기본값은 elasticsearch, 클러스터 이름이 같으면 노드들이 동일한<br/>클러스터로 구성된다.
node.master  | 마스터 노드 여부  | 기본값은 true, 마스터 서버 는 클러스터 상태 및 안정성을<담당한다. <br/>많은 클러스터 노드가있는 대규모 배포에서는 둘 이상의 전용 마스터를<br/>사용하는 것이 좋다.<br/>일반적으로 전용 마스터는 데이터를 저장하거나 인덱스를 만들지 않고,<br/>클러스터의 성능을 위험에 빠뜨릴 수있는 과부하가 없어야 한다.                      
